...
class Netpro2vec:
    ...
    uses probability distribution representations of graphs and skip-gram learning model.

    Args:
        **format** *(str, optional)* -  graph format. Default is "graphml"

        **dimensions** *(int, optional)* – number of features. Default is 128.

        **prob_type** *(list of str, optional)* –  list of probability types. Default is ["tm1"]
        (allowed values: "ndd", "fndd", "tm<walk-distance>").
        "ndd" means that a word for each node is formed by concatenating labels (or node ids) of nodes whose distance
        above a cut-off threshold with respect to the source node.
        "fndd" is a feature-weighted variant of "ndd".
        "tm<walf-distance>" means that a word for each node is formed by concatenating node labels (or node ids)
        which are reachable by the source node with a transition probability above a cut-off threshold.

        **cut_off** *(list of float, optional)* – list of cut-off thresholds to form words for each node. Default is
        [0.01]. Only the labels (or ids) of nodes with transition probability (for "tm<walk-distance>") or with
        distance (for "ndd"/"fndd") above this threshold are used for building the word.

        **agg_by** *(list of int, optional)* – list of *aggregators* in words formed with "ndd"/"fndd" annotation.
        Default is [0]. The number of bins used to build the Node Distance Distribution histogram is scaled down by
        ...
        A zero value means that this parameter does not apply to the annotation (i.e. "tm<walk-distance>").

        **extractor** *(list of int, optional)* – list of *extractor modes* in building words, one modality for each
        specified annotation. Default is [0]. Currently, only extractor 1 and 2 are supported.
        With respect to the corresponding annotation in the **prob_type** list:
        - extractor 1 returns words only from a single cut off.
        - extractor 2 returns multiple words of different lengths from different probability cut offs
          (The set of predefined cut offs is: 0, 0.1, 0.3, 0.5).
        ...
        **vertex_attribute** *(str, optional)*: name of the vertex attribute to be used as a label and/or for
        feature-weighted distributions (e.g. "fndd"). Default is None.

        **feature_sigma** *(float, optional)*: bandwidth parameter for Gaussian similarity in "fndd". Default is 1.0.

        **similarity** *(str, optional)*: similarity type for "fndd" ("gaussian" or "cosine"). Default is "gaussian".
    ...
    def get_documents(self, graphs: List[ig.Graph]):
        ...
        Return:
            The list of documents representing the graphs.
        """
    ...
    def infer_vector(self, graphs: List[ig.Graph], steps=None, alpha=None):
        ...
        Return:
            Embedding list inferred for the graphs.
        """
        probmats = self.__generate_probabilities_newsample(graphs)
        if self.vertex_attribute is not None:
            self.get_vertex_attributes(graphs)
        ...
        utils.vprint("Doc2Vec inferring (steps=%d, alpha%f) in progress..." % (steps, alpha),
    ...
    # for gensim 4.x compatibility (method overriding the previous one)
    def infer_vector(self, graphs: List[ig.Graph], epochs=None, alpha=None):
        """Inferring embedding method of Netpro2vec model (gensim 4.x signature)."""
        probmats = self.__generate_probabilities_newsample(graphs)
        if self.vertex_attribute is not None:
            self.get_vertex_attributes(graphs)
        docs = self.__get_document_collections_newsample(probmats, encodew=self.encodew)
        ...
        if alpha is None:
            alpha = self.learning_rate
        utils.vprint("Doc2Vec inferring (steps=%d, alpha%f) in progress..." % (epochs, alpha),
                     end='', verbose=self.verbose)
    ...
    def get_embedding(self):
        ...
        Return:
            The produced embedding in numpy array format (None if the model was not trained).
        """
    ...
    def __generate_probabilities(self, graphs: List[ig.Graph]):
        """Generate probability distributions for each requested prob_type."""
        if self.loadprobs:
            try:
    ...
    def __generate_probabilities_newsample(self, graphs: List[ig.Graph]):
        """Generate probability distributions for a new set of graphs (inference)."""
        probmats = {}
        for prob_type in self.prob_type:
            if prob_type == "fndd":
    ...
    def __batch_feature_extractor(self, probability_distrib_matrix, name, word_tag=None, tag=True,
        ...
        """Generate a document collection describing a single graph
           by concatenating distribution matrix data (NDD/fndd or TM<int>) for each node in the graph.
        """
        if self.vertex_attribute is not None:
    ...
    def __get_document_collections(self, workers=4, tag_doc=True, encodew=True):
        """Generate documents for graphs.
           ...
           distribution, then merged into a single vocabulary.
        """
        ...
        for prob_idx, prob_type in enumerate(self.prob_type):
            ...
            document_collections = [document_collections[x].graph_document
                                    for x in range(0, len(document_collections))]
            ...
            if tag_doc:
                for prob_type_doc in document_collections_all:
                    self.document_collections_list.append([
                        ...
                        for x in range(0, len(prob_type_doc))])
            else:
    ...
    def __get_document_collections_newsample(self, probmats, workers=4, tag_doc=True, encodew=True):
        """Generate documents for graphs (new sample)."""
